...
public class FreeBoardListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			List<FreeBoard> list = new BoardService().selectBoardListAll(cPage, numPerPage);
			//게시판 글목록을 담을 list
			
			
			
			int totalBoardCount = new BoardService().selectBoardCount();
			//(공식2)전체페이지수 구하기
			...
			while(!(pageNo>pageEnd || pageNo > totalPage)){
				...
				if(cPage == pageNo ){
					...
					pageBar += "<a class='cPage' href='"+request.getContextPath()+"/board/freeBoard?cPage="+pageNo+"'>"+pageNo+"</a> ";
				}
				pageNo++;
			}
			...
			request.getRequestDispatcher("/WEB-INF/views/board/freeboard/freeBoardList.jsp").forward(request, response);
